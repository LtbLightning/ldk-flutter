...
use crate::types::{ChainMonitor, ChannelManager, HTLCStatus, InvoicePayer, MillisatAmount, NetworkGraph, PaymentInfo, PaymentInfoStorage, PeerManager};
...
lazy_static! {
    static ref BITCOIND_CLIENT: RwLock<Arc<BitcoindClient>> = RwLock::new(Arc::new(BitcoindClient::default()));
    static ref KEYS_MANAGER: RwLock<Arc<KeysManager>> = RwLock::new(Arc::new(KeysManager::new(&[0; 32], 0, 0)));
...
async fn init_bitcoin_client(host: String, port: u16, username: String, password: String) {
    ...
    let mut client = BITCOIND_CLIENT.write().unwrap();
...
async fn init_channel_manager( node_network:Network,
                               persister: Arc<FilesystemPersister>,
                               chain_monitor: Arc<ChainMonitor>,
                               keys_manager: Arc<KeysManager>,
                               logger:Arc<FilesystemLogger>,
                               ldk_data_dir:String) -> (ChannelManager, Vec<(BlockHash, ChannelMonitor<InMemorySigner>)>,BlockHash)
{
    let bitcoind_client = BITCOIND_CLIENT.read().unwrap();
    let mut user_config = UserConfig::default();
    // Step 7: Read ChannelMonitor state from disk
    let mut channelmonitors:Vec<(BlockHash, ChannelMonitor<InMemorySigner>)> = persister.read_channelmonitors(keys_manager.clone()).unwrap();
...
pub async fn ldk_init(
    host: String, port: u16, username: String, password: String, storage_dir_path:String, network:String
){
    // Initialize the LDK data directory if necessary.
    init_directory(storage_dir_path);
    ...
    let (channel_manager, mut channelmonitors, channel_manager_blockhash) =  init_channel_manager(node_network, persister.clone(),chain_monitor.clone(),keys_manager.clone(),   logger.clone(), ldk_data_dir.clone()).await;
   ...
   //  *channel_manager_stat = Some(Arc::new(channel_manager));
    ...
    if check_if_restarting(ldk_data_dir.clone()) {
        ...
        let mut chain_listeners = vec![(channel_manager_blockhash, &channel_manager as &dyn chain::Listen)];
    ...
    let network_graph = Arc::new(file_io::read_network(Path::new(&network_graph_path), genesis, logger.clone()));
